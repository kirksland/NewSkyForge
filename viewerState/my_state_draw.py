...
class State(BaseState):
    def __init__(self, **kwargs):
        ...
        self.state_name = kwargs.get('state_name', 'my_state_draw')
        ...
        self.f0 = F.HoverGadgetFeature(line_gadget='line_gadget', face_gadget='face_gadget', point_gadget='point_gadget', point_hover_gadget='point_hover_gadget', enable_ray_filter=True, write_ctx_hover=True)
        self.f1 = F.HoverMoveFeature()
        self.f2 = F.PreviewFeature(prefix='preview', enable_hover=False)
        self.hub = FeatureHub([self.f0, self.f1, self.f2])

    def onEnter(self, kwargs):
        self.bind_context(self.ctx, kwargs, ensure_geo=True, ensure_mesh=True)
        self.ctx.ensure_edit_geo()
        self.f0.set_mode('line')
        self.f0.set_allowed_modes(['line', 'point', 'face'])
    ...
    def onDraw(self, kwargs):
        if self.ctx.edit_geo is not None and self.f0.geometry is not self.ctx.edit_geo:
            self.f0.set_geometry(self.ctx.edit_geo)
...
def createViewerStateTemplate():
    state_typename = 'my_state_draw'
    state_label = 'my_state_draw'
    state_cat = hou.sopNodeTypeCategory()

    template = hou.ViewerStateTemplate(state_typename, state_label, state_cat)
    template.bindFactory(State)
    feature_classes = [F.HoverGadgetFeature, F.HoverMoveFeature, F.PreviewFeature]
    ...
    for cls in feature_classes:
        ...
        if menu is None and hasattr(cls, 'build_menu'):
            ...
            continue
        if menu is not None and hasattr(cls, 'extend_menu'):
